...
__attribute__((const)) char is_terminal_task_id(int task_id) { return task_id == -1; }
...
void* solve_single(void* arg) {
    ...
    algorithm_data_t algorithm_data = {
        ...
        .empty = (char*)alloca(buffer_size * sizeof(char)),
        .filled = (char*)alloca(buffer_size * sizeof(char)),
        .unknown = (char*)alloca(buffer_size * sizeof(char)),
        .positions = (int*)alloca(buffer_size * sizeof(int)),
        .suggest = (char*)alloca(buffer_size * sizeof(char)),
        .updated = false,
        .skipped = false,
    };

    memset(algorithm_data.empty, ' ', buffer_size);
    ...
    while(true) {
        ...
        // Skip this task if it is already solved (minor optimization).
        ...
        thread_data->field_updated = thread_data->field_updated || algorithm_data.updated;
        thread_data->rows_skipped = thread_data->rows_skipped || algorithm_data.skipped;
